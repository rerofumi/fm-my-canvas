...
本ドキュメントは `docs/03_agent_update.md` v2.0.0 の Phase 2 を実装するための具体仕様書である。
Phase 1 (`docs/04_agent_specification.md`) が完了していることを前提とする。

**ステータス**: ✅ Phase 2 主経路 実装完了（2026-04-05 確認済み）
...
- ~~`apply_diff` はオプションであり、無効化していても `apply_edit` + `write_file` で支障なく動作する~~ → `apply_diff` は実装見送り（延期）
...
- ✅ Phase 1 の受け入れ条件「Tool Call メッセージがセッション履歴に保存されること」が満たされている
- ✅ `ToolManager.Tools()` の返却順が map 依存で非決定的 → 現状許容（将来的に安定化が必要な場合は sorted keys を使用）
- ✅ `apply_diff` はデフォルト無効とし、`apply_edit` を Phase 2 の主経路とする → **実装見送り（延期）**
...
新規 (❌ 延期):
  tools/diff_engine.go             (延期: apply_diff 未実装)
  tools/diff_engine_test.go        (延期)
  tools/diff_apply_tool.go         (延期)
  tools/diff_apply_tool_test.go    (延期)

変更 (✅ 実装済み):
  chat.go
    ... apply_edit の Tool 登録追加
    ... buildSystemPrompt の Phase 2 更新 (apply_edit 追加)
    ... context window 管理強化 (summarizeOldToolResults)

  tools/registry.go
    ... Tools() の返却順序は未安定化 (map iteration)
```
...
## Step 1: `tools/edit_engine.go` — ✅ 実装済み
...
var errFileSizeLimit = fmt.Errorf("file size exceeds 1MB limit")
...
> **設計からの逸脱**: 当初の仕様では `EditResult` 構造体 (`Content`, `MatchCount`, `Replacement`) を返す予定だったが、実装では `(string, error)` の直接戻り値に簡略化した。`MatchCount` は `FindMatchCount` メソッドで別途取得可能。
...
## Step 2: `tools/edit_apply_tool.go` — ✅ 実装済み
...
## Step 3: `tools/diff_engine.go` — ❌ 延期

> **延期理由**: ツール数増加による LLM 混乱を防ぐため、`apply_edit` + `write_file` の 2 本に絞る方針に決定。将来的に `apply_diff` が必要になれば追加を検討する。
...
## Step 4: `tools/diff_apply_tool.go` — ❌ 延期
...
### 5.2 System Prompt の Phase 2 更新 — ✅ 完了

Agent モードの System Prompt に `apply_edit` の説明を追加する。以下は実際の実装コード:
...
5. ✅ 長時間ループで古い Tool 結果が要約される（`summarizeOldToolResults` による）
...
**バージョン**: 2.0.0  
**作成日**: 2026-04-05  
**最終更新**: 2026-04-05 (Phase 2 主経路 実装完了を反映: apply_edit + EditEngine 完了, apply_diff 延期明記, EditResult 簡略化を追記)
**対象フェーズ**: Phase 2 (Diff-Based Edit) — ✅ 主経路完了 / ❌ apply_diff 延期
