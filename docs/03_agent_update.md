...
#### 3.3.5. `apply_diff` - 差分パッチ適用 (Phase 2, オプション)

**目的**: 既存コードへの差分パッチを適用し、最小変更でコードを更新。
...
func (t *ApplyDiffTool) Execute(sessionID string, args map[string]any) (string, error) {
    ...
    patched, err := unified.Apply(content, []byte(diff))
    if err != nil {
        return "", fmt.Errorf("failed to apply diff: %w", err)
    }
    
    if err := t.manager.WriteFile(sessionID, path, string(patched)); err != nil {
        return "", err
    }
    
    return fmt.Sprintf("Successfully applied diff to %s", path), nil
}
```
...
2. **Tool レジストリ** (`tools/registry.go`):
   ...
   func (tm *ToolManager) ExecuteToolCall(sessionID string, toolCall types.ToolCall) (string, error)
   ```
...
5. **ChatService の拡張** (`chat.go`):
   ...
   // sendMessageWithTools は非公開メソッドとして Tool Call ループを実行
   func (c *ChatService) sendMessageWithTools(sessionID string, messages []types.Message) error
   ```
...
1. **Tool Call 可視化** (`components/chat/ToolCallMessage.svelte`):
   ...
   - 実行結果を折りたたみ表示
   - **Tool Call ループ中のリアルタイムステータス表示**:
     - `tool-event (tool_call)` 受信時: 「📖 Reading index.html...」のような実行中インジケータ
     - `tool-event (tool_result)` 受信時: 成功/失敗ステータスとサマリ表示
     - 複数ラウンドの進行状況をインジケータで可視化

2. **設定 UI** (`components/layout/SettingsModal.svelte`):
   - "Agent モード" の切替スイッチ追加
   - Tool 使用のオン/オフ設定

3. **Event Handler の拡張** (`lib/services/chat.ts`):
   - `tool-event` を追加し、Tool 実行状態を配信する
   - **互換方針**: 既存の `llm-event` (`chunk` / `done` / `error`) は `map[string]string` のまま維持し、破壊的変更を避ける

   **イベントプロトコル更新（後方互換あり）**:

   | イベント | 現在 | 変更後 |
   |---------|------|--------|
   | `llm-event` (chunk) | `{type, content, session_id} map[string]string` | 変更なし |
   | `llm-event` (done) | `{type, content, session_id} map[string]string` | 変更なし |
   | `llm-event` (error) | `{type, content, session_id} map[string]string` | 変更なし |
   | `tool-event` (tool_call) | なし | `{type:"tool_call", tool_name, tool_args, session_id} map[string]any` |
   | `tool-event` (tool_result) | なし | `{type:"tool_result", tool_name, result, session_id} map[string]any` |
   | `artifact-update` | `{session_id, preview_url, files} map[string]string` | 変更なし (Tool 実行後も同一形式で emit) |
...
func (c *ChatService) RestoreArtifacts(sessionID string) map[string]string {
    ...
    // Agent モード / Markdown モード共通:
    // ファイルシステム上の Artifact 一覧から復元する
...
4. **フォールバック戦略**:
   - `apply_edit` または `apply_diff` の適用失敗時、エラーを LLM にフィードバック
   - LLM が `write_file` で完全再生成を試みることを期待
...
├── main.go
├── app.go
├── chat.go                  # sendMessageWithTools 追加, モード分岐
├── provider/
│   ├── provider.go          # StreamEvent, ToolDefinition 型追加, StreamWithTools 追加
│   ├── ollama.go            # StreamWithTools 実装 (NDJSON tool_calls 解析)
│   └── openrouter.go        # StreamWithTools 実装 (SSE delta.tool_calls 解析)
├── artifacts/
│   ├── manager.go           # ReadFile 追加, パスバリデーション強化
│   └── server.go
├── tools/                   # 新規
...
│   ├── file_write_tool.go
│   ├── file_list_tool.go
│   ├── edit_apply_tool.go   # Phase 2: search/replace 編集
│   ├── diff_apply_tool.go   # Phase 2: unified diff (オプション)
│   ├── code_search_tool.go  # Phase 3
│   ├── edit_engine.go       # search/replace 適用ロジック
│   └── diff_engine.go       # 差分適用ロジック (オプション)
├── session/
├── config/
│   └── config.go            # AgentMode フィールド追加
├── types/
│   └── types.go             # ToolCall, RoleTool 追加
├── frontend/
│   └── src/
│       ├── lib/
│       │   ├── services/
│       │   │   └── wails.ts          # tool_call/tool_result イベントリスナー追加
│       │   ├── stores/
│       │   │   └── chat.svelte.ts    # toolCallLog state 追加
│       │   └── parsers/
│       │       └── artifact.ts
│       └── components/
│           ├── chat/
│           │   ├── ToolCallMessage.svelte  # 新規: Tool Call 表示
│           │   └── ...
│           └── layout/
...
1. **ユーザーキャンセル**: Tool Call ループ中にユーザーがキャンセルした場合、`context.Cancel()` により現在の Tool 実行および LLM リクエストを中断する
2. **個別 Tool タイムアウト**: 各 Tool 実行に 30 秒のタイムアウトを設定。超過時はエラーとして LLM にフィードバック
3. **全体タイムアウト**: 1 回のメッセージに対する Tool Call ループ全体に 5 分のタイムアウトを設定
...
func (c *ChatService) sendMessageWithTools(sessionID string, messages []types.Message) error {
    ...
    for round := 0; round < maxToolRounds; round++ {
        ...
        // 個別 Tool 実行にもタイムアウトを適用
        toolCtx, toolCancel := context.WithTimeout(ctx, 30*time.Second)
        result, err := toolManager.ExecuteWithContext(toolCtx, sessionID, tc)
        toolCancel()
    }
}
```
...
| 対策 | 説明 |
|------|------|
| **Tool 結果の切り詰め** | `read_file` の結果が一定トークン数を超える場合、先頭 + 末尾のみを保持し中間を省略 |
| **古い Tool 結果の除外** | LLM に送信するメッセージ構築時、直近 N ラウンド以外の Tool 結果メッセージの Content を要約に置換 |
| **System Prompt での指示** | 「大きなファイルは必要な部分のみ `read_file` せよ」と指示（将来的に行範囲指定の `read_file` 拡張を検討） |

Phase 1 では **Tool 結果の切り詰め** (最大 50KB) のみ実装し、他は Phase 2 以降で対応する。
...
### フェーズ 1 (MVP)
- [ ] `types.Message` 拡張 (`ToolCalls`, `ToolCallID`, `RoleTool`)
- [ ] `artifacts.Manager.ReadFile` 追加
- [ ] `artifacts.Manager.WriteFile` / `ListFiles` に共通パスバリデーション適用
- [ ] `config.Config` に `AgentMode` 追加
- [ ] `provider.StreamEvent` / `ToolDefinition` 型定義
- [ ] Tool インターフェース定義
- [ ] `read_file`, `write_file`, `list_files` 実装
- [ ] Provider の `StreamWithTools` 実装 (Ollama + OpenRouter)
- [ ] ChatService のモード分岐 (`SendMessage` 内で `AgentMode` 判定)
- [ ] `tool-event` 追加（`llm-event` 互換維持）
- [ ] Tool Call ループ中のリアルタイムステータス表示
- [ ] `RestoreArtifacts` の Agent モード対応
- [ ] Tool 結果の切り詰め (最大 50KB)
- [ ] フロントエンドの Tool Call 可視化
- [ ] フロントエンドの Agent モード切替 UI

#### フェーズ 1 の非対象（明示）
- `apply_edit` / `apply_diff` / Edit Engine / Diff Engine
- `search_code` / コードインデックス
- 並列 Tool 呼び出し最適化

### フェーズ 2
- [ ] `apply_edit` Tool 実装 (search/replace 方式、主要)
- [ ] `apply_diff` Tool 実装 (unified diff 方式、オプション)
- [ ] Edit Engine / Diff Engine の実装
- [ ] 差分適用失敗時の `write_file` フォールバック機構
- [ ] Tool 結果の context window 管理強化
...
- `docs/04_agent_specification.md` — 本ドキュメント Phase 1 の実装仕様書。行レベルの変更内容とテストを定義

> **注意**: 本ドキュメント (03) を更新した場合、`04_agent_specification.md` も連動して更新すること。

---

**バージョン**: 1.3.0  
**作成日**: 2026-04-04  
**最終更新**: 2026-04-04 (レビュー反映: API統一, RestoreArtifacts対応, apply_edit追加, キャンセル/タイムアウト, context window管理)
**ステータス**: 提案中
